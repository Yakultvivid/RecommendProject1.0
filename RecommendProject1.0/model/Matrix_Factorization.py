...
class MatrixFactorization(object):
    ...
    def fit(self, user_matrix):
        ...
        self.user_matrix = user_matrix.values
        length, wide = self.user_matrix.shape
        self.P = np.random.rand(length, self.K)
        self.Q = np.random.rand(wide, self.K)

        self.r_index = self.user_matrix.nonzero()
        self.r = self.user_matrix[self.r_index[0], self.r_index[1]]
    ...
    def _comp_descent(self, index):
        ...
        e_ij = self.user_matrix[r_i, r_j] - r_ij_hat
